...
export default {
  ...
  methods: {
    async getReuniao() {
        try { 
            const response = await apiClient.get('/trabalho/' + this.$route.params.id + '/reuniao/' + this.$route.params.idreuniao);
            this.reuniao = response.data.data;
        } catch(err) {
            console.log(err);
            return
        }
